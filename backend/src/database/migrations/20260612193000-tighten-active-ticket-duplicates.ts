  ...
  up: async (queryInterface: QueryInterface) => {
      ...
      WITH ranked AS (
        SELECT
          ...
          ROW_NUMBER() OVER (
            PARTITION BY
              t."companyId",
              COALESCE(t."channel", c."channel", 'whatsapp'),
              COALESCE(t."isGroup", c."isGroup", false),
              COALESCE(
        ...
        WHERE t."status" IN (${ACTIVE_STATUSES})
          AND COALESCE(t."channel", 'whatsapp') <> 'standalone'
      )
      UPDATE "Tickets" t
      SET
        "status" = 'closed',
    ...
    await queryInterface.sequelize.query(`
      ...
      DECLARE
        new_identity text;
        new_channel text;
        new_is_group boolean;
      BEGIN
        IF NEW."status" NOT IN (${ACTIVE_STATUSES}) THEN
          RETURN NEW;
        END IF;

        new_channel := COALESCE(NEW."channel", 'whatsapp');
        IF new_channel = 'standalone' THEN
          RETURN NEW;
        END IF;
        ...
        SELECT
          ...
          COALESCE(NEW."isGroup", c."isGroup", false)
        INTO new_identity, new_is_group
        FROM "Contacts" c
        ...
        IF EXISTS (
          ...
          WHERE t."id" <> COALESCE(NEW."id", 0)
            ...
            AND COALESCE(t."channel", c."channel", 'whatsapp') = new_channel
            AND COALESCE(t."isGroup", c."isGroup", false) = COALESCE(new_is_group, false)
            AND COALESCE(t."channel", 'whatsapp') <> 'standalone'
    ...
    await queryInterface.sequelize.query(`
      ...
      BEFORE INSERT OR UPDATE OF "status", "contactId", "whatsappId", "companyId", "quadroGroupId", "channel", "isGroup"
      ON "Tickets"
      FOR EACH ROW
