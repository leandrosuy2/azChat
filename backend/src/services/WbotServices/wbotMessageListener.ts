...
/**
 * Dedup cache em memória por (companyId, wid). Cobre a janela entre o evento
 * messages.upsert chegar e o INSERT bater no Postgres — a UNIQUE em (wid, companyId)
 * já é o gate definitivo, mas evitamos abrir transação por nada e damos
 * idempotência rápida no cluster do listener.
 *
 * TTL curto: o Baileys raramente re-emite o mesmo wid depois de alguns segundos,
 * exceto durante messaging-history.set, onde o ImportWhatsAppMessageService
 * controla manualmente.
 */
const WID_DEDUP_TTL_MS = 60_000;

function createWidDedupCache() {
  ...
  return {
    ...
    add(key: string) {
      map.set(key, Date.now() + WID_DEDUP_TTL_MS);
      // GC oportunista quando a tabela cresce muito.
      if (map.size > 4096) {
        const now = Date.now();
    ...
    // Permite que o handler retorne a mensagem para reprocessamento se o
    // processamento atual falhar (Redis fora, DB intermitente, etc.).
    delete(key: string) {
      map.delete(key);
    }
  };
...
const processedWidCache = createWidDedupCache();
...
const downloadMedia = async (
  ...
  // Mensagens sem mediaKey vêm tipicamente do messaging-history.set (sincronização
  // inicial) e nunca vão decifrar — pular silenciosamente em vez de poluir o log
  // com stack trace. Era a fonte das ~200 ocorrências de "Cannot derive from empty media key".
  if (!(message as any)?.mediaKey || !(message as any)?.mediaKey?.length) {
    if (!isImported) {
      logger.debug(
        `downloadMedia: msg ${msg.key?.id} sem mediaKey, ignorando download.`
      );
    }
    return null;
  }
  ...
  // Patch da URL antiga do sticker: clonamos antes de mutar para não corromper
  // o payload original que ainda é referenciado por outros handlers concorrentes.
  let msgForDownload: WAMessage = msg;
    ...
    if (msg.message?.stickerMessage?.url?.includes(urlAnt)) {
      msgForDownload = {
        ...
        message: {
          ...
          stickerMessage: {
            ...msg.message.stickerMessage,
            url: msg.message.stickerMessage.url.replace(urlAnt, final)
          }
        }
      } as WAMessage;
    }
  ...
  try {
    ...
    // Log enxuto: uma linha por falha, sem stack trace.
    const reason = err?.message || String(err);
    if (isImported) {
      logger.warn(`[media imp] msg=${msg.key?.id} indisponível: ${reason}`);
    } else {
      logger.warn(
        `[media] download falhou msg=${msg.key?.id} tipo=${getTypeMessage(msg)} motivo=${reason}`
      );
    }
    return null;
  }
...
export const verifyMediaMessage = async (
  ...
  try {
    ...
    if (!media) {
      // Mídia ao vivo (não importada) que falhou no download — em vez de descartar
      // a mensagem (que era o comportamento anterior), persistimos um placeholder
      // de texto. Assim o atendente vê que "algo chegou" e a deduplicação por wid
      // continua válida, evitando reprocessamentos repetidos.
      const placeholderBody =
        getBodyMessage(msg) ||
        "*Sistema:* Mídia recebida não pôde ser baixada (chave ausente ou expirada).";

      const messageDataFallback = {
        wid: msg.key.id,
        ticketId: ticket.id,
      ...
      await ticket.update({ lastMessage: placeholderBody });
      return CreateMessageService({
        messageData: messageDataFallback,
        companyId
      });
    }
...
const wbotMessageListener = (wbot: Session, companyId: number): void => {
  wbot.ev.on("messages.upsert", async (messageUpsert: ImessageUpsert) => {
    ...
    // Processamento sequencial: antes era forEach(async) que disparava N promessas
    // paralelas, causando duplicação no DB e exaustão do pool do Postgres em picos.
    // A paralelização real fica agora para o Bull (concorrência por sessão).
    for (const message of messages) {
     try {
      if (
        ...
        logger.warn(
          `[wpp=${wbot.id}] MENSAGEM PERDIDA wid=${message.key?.id} remoteJid=${message.key?.remoteJid}`
        );
      }

      const widKey = `${companyId}:${message.key.id}`;
      // Dedup em memória (TTL 60s) — protege contra eventos repetidos antes
      // do INSERT bater no Postgres. A UNIQUE em (wid, companyId) é o gate final.
      if (processedWidCache.has(widKey)) {
        continue;
      }
        ...
        if (!isCampaign) {
          if (REDIS_URI_MSG_CONN !== "") {
            try {
              ...
              // Bull/Redis indisponível → fallback inline para não perder a msg.
              Sentry.captureException(e);
              logger.warn(
                `Bull indisponível, processando inline wid=${message.key.id}`
              );
      ...
      Sentry.captureException(err);
      logger.error(
        `[wpp=${wbot.id}] Erro em messages.upsert wid=${message?.key?.id}: ${(err as Error)?.message}`
      );
      // Libera o cache para que o próximo evento idêntico possa retry-ar.
      // Sem isso, uma falha transitória (Redis/DB blip) marcava a mensagem como
      // "vista" por 60s e ela nunca seria reprocessada → "mensagem não chegou".
      if (message?.key?.id) {
        processedWidCache.delete(`${companyId}:${message.key.id}`);
      }
      // Não relança: uma mensagem com defeito não pode parar o batch inteiro.
     }
    }
  });
    ...
    messageUpdate.forEach(async (message: WAMessageUpdate) => {
     try {
      (wbot as WASocket)!.readMessages([message.key]).catch(err => {
        logger.warn(
          `Could not send read receipt on message update. Err: ${err?.message || err}`
        );
      });
      ...
      logger.warn(
        `messages.update falhou wid=${message?.key?.id}: ${(err as Error)?.message}`
      );
     }
    });
  });
  ...
  wbot.ev.on("contacts.update", (contacts: any) => {
    contacts.forEach(async (contact: any) => {
     try {
      ...
      // Não deixa que uma falha em update de contato derrube o processo
      // (era um dos gatilhos do unhandledRejection → exit(1)).
      logger.warn(
        `contacts.update falhou contact=${contact?.id}: ${(err as Error)?.message}`
      );
     }
    });
  });
  wbot.ev.on("groups.update", (groupUpdate: GroupMetadata[]) => {
    ...
    groupUpdate.forEach(async (group: GroupMetadata) => {
     try {
      ...
      logger.warn(
        `groups.update falhou group=${group?.id}: ${(err as Error)?.message}`
      );
     }
    });
  });
